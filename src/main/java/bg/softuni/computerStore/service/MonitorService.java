...
public class MonitorService implements InitializableProductService {
    ...
    public void init() {
        if (allItemsRepository.findCounItemsByType("monitor") < 1) {
            initOneMonitor("Monitor 23.8\" Dell PE 2422EE", 600.25, 650.55, 8,
            ...
            initOneMonitor("Monitor 23.8\" Philips 243V7QDSB", 250.25, 330.22, 9,
                    "23.8\" (60.5 cm)", "1920 x 1080", "IPS", "178/178",
                    "75 Hz", "250 cd/m2", "Интерфейси: VGA, 1 x HDMI, 1 x DVI-D порт, 1 x Audio Out");
        }
    }

    private void initOneMonitor(String name, double buyAt, double sellAt, int newQuantity,
                                String sizeInInches, String resolution, String matrixType, String viewAngle,
                                String refreshRate, String brightness,
                                String moreInfo) {

        MonitorEntity toAdd =
                new MonitorEntity(name, BigDecimal.valueOf(buyAt), BigDecimal.valueOf(sellAt), newQuantity);
